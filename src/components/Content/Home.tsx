...
export default function Home() {
  ...
  useEffect(() => {
    const handleMQTTMessage = (topic: string, payload: Buffer) => {
      ...
      } else if (topic === 'kapasitas_air') {
        setWaterCapacity(message)
      }
    }
  ...
  const getWaterCapacityStatus = (capacity: number) => {
    if (capacity < 30)
      return {
        icon: AlertTriangle,
        message: 'Kapasitas air rendah, segera isi ulang.',
        color: 'text-red-400',
        bgGradient: 'from-red-500/20 to-red-600/10',
        borderColor: 'border-red-500/30',
      }
    if (capacity < 60)
      return {
        icon: TrendingUp,
        message: 'Kapasitas air sedang, tetap pantau.',
    ...
    return {
      icon: CheckCircle,
      message: 'Kapasitas air mencukupi.',
      color: 'text-emerald-400',
      bgGradient: 'from-emerald-500/20 to-emerald-600/10',
      borderColor: 'border-emerald-500/30',
    }
  ...
  const waterCapacityStatus = getWaterCapacityStatus(waterCapacity)
    ...
    >
      <div className='relative z-10'>
        ...
        <div className='flex items-start gap-2 mt-1'>
          <status.icon
            className={`w-4 h-4 mt-0.5 ${status.color} flex-shrink-0`}
          />
      ...
      <div className='mt-8'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 overflow-auto'>
          ...
          <MetricCard
            ...
            value={waterCapacity}
            unit='%'
            percentage={waterCapacity}
            status={waterCapacityStatus}
          />
        </div>
